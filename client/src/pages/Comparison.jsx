      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className={theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}>
            Track your productivity changes over time
          </p>
        </div>
        ...
        <div className={`p-6 rounded-xl ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'} mb-8`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {weekComparison.map((item, idx) => (
              <div key={idx} className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                <p className={`text-sm mb-2 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{item.metric}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className={`p-6 rounded-xl ${theme === 'dark' ? 'bg-gradient-to-br from-green-900/50 to-emerald-900/50 border border-gray-700' : 'bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200'}`}>
            <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
              <TrendingUp className="text-green-500" />
              Best Day This Week
            </h3>
            <div className="text-3xl font-bold mb-2">{bestWorstDays.best.day}</div>
            <div className={`text-lg mb-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
              {bestWorstDays.best.time} active
            </div>
            <div className="text-2xl font-semibold text-green-500">
              Score: {bestWorstDays.best.score}%
            </div>
          </div>

          <div className={`p-6 rounded-xl ${theme === 'dark' ? 'bg-gradient-to-br from-red-900/50 to-orange-900/50 border border-gray-700' : 'bg-gradient-to-br from-red-50 to-orange-50 border border-red-200'}`}>
            <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
            ...
            <div className={`text-lg mb-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
              {bestWorstDays.worst.time} active
            </div>
            <div className="text-2xl font-semibold text-red-500">
              Score: {bestWorstDays.worst.score}%
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-white/50'}`}>
              ...
              <p className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>
                Your productivity has increased by 14% this month. You're consistently hitting your peak performance between 9 AM and 12 PM.
              </p>
            </div>
            <div className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-white/50'}`}>
              <h4 className="font-semibold mb-2">Distraction Pattern</h4>
              <p className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>
                Social media usage has decreased by 12% compared to last month. Keep up the good work! Consider maintaining this pattern.
              </p>
            </div>
            <div className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-white/50'}`}>
              <h4 className="font-semibold mb-2">Recommendation</h4>
              <p className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>
                Your Sunday productivity is 45% lower than your weekly average. Try scheduling lighter tasks or taking intentional breaks on this day.
              </p>
            </div>
          </div>
        </div>
